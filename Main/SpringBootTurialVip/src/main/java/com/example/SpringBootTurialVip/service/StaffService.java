...
public class StaffService {
    ...
    // 1️⃣ Lấy danh sách tất cả `Child` (Không giới hạn Parent)
    ...
    // 2️⃣ Lấy danh sách tất cả `Parent`
    @PreAuthorize("hasRole('STAFF')")
    ...
    // 3️⃣ Tạo `Child` cho `Parent` bất kỳ
    @PreAuthorize("hasRole('STAFF')")
    public ChildResponse createChildForParent(Long parentId, ChildCreationRequest request) {
        User parent = userRepository.findById(String.valueOf(parentId))
    ...
    // 4️⃣ Cập nhật thông tin `Child` của bất kỳ `Parent`
    @PreAuthorize("hasRole('STAFF')")
    public ChildResponse updateChildInfo(Long childId, ChildCreationRequest request) {
        User child = userRepository.findById(String.valueOf(childId))
